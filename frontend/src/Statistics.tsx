...
const Statistics = () => {
  ...
  useEffect(() => {
    ...
    const fetchFarms = async () => {
      const response = await axios.get(`http://${backendHost}:8091/farms`)
  ...
  useEffect(() => {
    ...
    const fetchStatList = async () => {
      const response = await axios.get(`http://${backendHost}:8091/data?farm=${farmId ?? ''}&metrictype=${metricType ?? ''}&month=${month ?? ''}&startdate=${startDate ?? ''}&enddate=${endDate ?? ''}`)
